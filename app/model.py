...
SEGMENTS_DIR = Path("../segments/")  # Target folder for saving audio segments
...
def segment_audio(audio, segment_duration=5):
    segments = []
    segment_samples = int(segment_duration * SAMPLE_RATE)
    total_samples = len(audio)
    total_segments = (total_samples + segment_samples - 1) // segment_samples  # Ceiling division for complete coverage

    for i in range(total_segments):
        start = i * segment_samples
        end = start + segment_samples
        if end > total_samples:
            # Pad the last segment if it is shorter than segment_samples
            segment = np.pad(audio[start:], (0, end - total_samples), mode='constant')
        else:
            segment = audio[start:end]
        
        # Save the segment to a file
        segment_filename = SEGMENTS_DIR / f"segment_{i+1:03d}.wav"
        sf.write(segment_filename, segment, SAMPLE_RATE)
        print(f"Saved segment {i+1} to {segment_filename}")

        segments.append((start / SAMPLE_RATE, min(end, total_samples) / SAMPLE_RATE, segment))
